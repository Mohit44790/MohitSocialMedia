...
const LeftSidebar = () => {
  ...
  const logoutHandler = async () => {
    try {
      const res = await axios.get(
        "https://mohitsocialmedia.onrender.com/api/v1/user/logout",
        {
  ...
  return (
    <>
      {/* Mobile Menu Button */}
      <div className="fixed top-2 left-5 z-20 md:hidden">
        <button
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 z-10 h-screen w-58 bg-gray-100 border-r border-gray-300 p-4 transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex flex-col items-start space-y-6">
          ...
          <div className="mt-8 space-y-7">
            {(user ? sidebarItems : guestItems).map((item, index) => (
              <div
                key={index}
